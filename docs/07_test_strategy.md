...
| CI | なし | 今回は省略 |
| Keep-alive Worker | `make keepalive-check` | Wrangler type generation、TypeScript、Workers runtime unit tests、deploy dry-run |
...
運用 Worker を変更した場合は追加で以下を残す。

- `make keepalive-check` の結果
- migration の権限（anon SELECT only）の確認
- 本番反映時は Cron Event と `supabase.keepalive.succeeded` log の確認
